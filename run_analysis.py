...
# 리포트 생성
# ════════════════════════════════════════════════════════════
def signal_row(ticker: str, name: str, sig: dict) -> str:
    """마크다운 테이블 한 줄."""
    s = sig["signals"]
    icons = {k: ("✅" if v == "매수" else ("❌" if v == "매도" else "➖")) for k, v in s.items()}
    return (
...
def hmm_feature_row(ticker: str, name: str, sig: dict) -> str:
    rz  = sig["returns_z"]
    raz = sig["range_z"]
    vcz = sig["vol_change_z"]
    # 간단한 regime 추정 (HMM 모델 없을 때 휴리스틱)
    if rz > 1.0 and raz < 0.5:
        ...
        regime = "🌀 노이즈"
    return f"| {ticker} | {name} | {rz:+.2f} | {raz:+.2f} | {vcz:+.2f} | {regime} |"
...
def build_report(kr_results: dict, us_results: dict) -> str:
    ...
    lines = [
        ...
        f"",
        f"> 생성일시: {now}  ",
        f"> ⚠️ 본 리포트는 **보조 참고용**입니다. 투자 판단은 본인 책임입니다.",
        f"",
        f"---",
        f"",
    ]
    ...
    for ticker, (name, sig) in kr_results.items():
        if sig:
            lines.append(signal_row(ticker, name, sig))
    ...
    for ticker, (name, sig) in kr_results.items():
        if sig:
            lines.append(hmm_feature_row(ticker, name, sig))
    ...
    lines += [
        ...
        "---",
        "",
        f"*자동 생성: [quant-pipeline](https://github.com/your-repo/quant-pipeline)*",
    ]
